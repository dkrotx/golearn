...
func (t *Task) ListToDo(limit int, reply *ToDoListReply) error {
    var res ToDoListReply

    log.Printf("Called ListToDo: limit=%d", limit)

    if len(todoSlice) < limit {
        limit = len(todoSlice)
    }

    for i := 0; i < limit; i++ {
        res.TaskList = append(res.TaskList, todoSlice[i])
    }
...
func (t *Task) MakeToDo(todo ToDo, reply *ToDo) error {
    log.Printf("Adding new ToDo: %s", todo.Title)
